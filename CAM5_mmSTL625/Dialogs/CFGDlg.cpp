...
CCFGDlg::CCFGDlg(int size, CWnd* pParent /*=NULL*/)
...
void CCFGDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MinMaxDouble(pDX, m_max, -1000000000, 1000000000);
	DDX_Text(pDX, IDC_EDIT2, m_min);
	DDV_MinMaxDouble(pDX, m_min, -1000000000, 1000000000);
}


BEGIN_MESSAGE_MAP(CCFGDlg, CDialogEx)
	ON_BN_CLICKED(IDAPPLY, &CCFGDlg::OnApply)
	ON_WM_DRAWITEM()
END_MESSAGE_MAP()
...
void CCFGDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
...
void CCFGDlg::AddPt(double x, double y)
{
	if( m_n < m_size )
	{
		m_ps[m_n][0] = x ;
...
void CCFGDlg::GetBox(double min[2], double max[2])
{
	for( int k = 0 ; k < 2 ; k++ )
	{
		min[k] = MAX_DBL ;
...
void CCFGDlg::SetMax(double max)
{
	m_max = max ;
}

void CCFGDlg::SetMin(double min)
{
	m_min = min ;
}
